...
### Fixed
- **`LoginProps::language`** — `#[prop_or_default]` so apps can omit it when using explicit prompt strings.
...
- **`axum-extra` 0.10 → 0.12** — align Cookie types with all studio2201 apps so
  `cookie_auth` builders work with app `CookieJar`s without dual-version graphs.
