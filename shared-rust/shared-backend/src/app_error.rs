//! Generic application error envelope.
//!
//! Handlers return `Result<impl IntoResponse, AppError>` so the framework
//! can render a consistent error body without each handler rolling its
//! own JSON. The `IntoResponse` impl intentionally omits internal error
//! details from the user-facing body to avoid leaking paths, secrets, or
//! stack traces to clients.

use axum::http::StatusCode;
use axum::response::{IntoResponse, Response};

/// Generic application error type. Apps typically extend this via
/// `#[error("...")]` variants and `#[from]` impls for their own
...
mod tests {
    ...
    fn into_response_redacts_internal_details() {
        let resp = AppError::internal("db password leaked into message").into_response();
